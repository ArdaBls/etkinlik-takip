(() => {
  ...
  const renderUsers = () => {
    ...
    const profiles = [...userDirectory.values()].sort((first, second) => Number(first.approved) - Number(second.approved) || first.email.localeCompare(second.email, "tr"));
  ...
  const renderAuditLogs = (snapshot) => {
    ...
    const labels = { "event.create": "Etkinlik eklendi", "event.update": "Etkinlik güncellendi", "event.delete": "Etkinlik silindi", "home.create": "Çocuk evi eklendi", "home.rename": "Çocuk evi adı değiştirildi", "home.delete": "Çocuk evi silindi", "home.photo.update": "Sorumlu fotoğrafı güncellendi", "user.approve": "Kullanıcıya izin verildi", "user.revoke": "Kullanıcı izni geri alındı", "user.block": "Kullanıcı engellendi", "user.unblock": "Kullanıcı engeli kaldırıldı" };
  ...
  const initTheme = () => {
    const saved = localStorage.getItem("etkinlik-takip-theme");
    if (saved === "dark") document.body.classList.add("dark-theme");
    const meta = $("#theme-color-meta");
    const update = () => {
      const dark = document.body.classList.contains("dark-theme");
      meta.content = dark ? "#12211f" : "#f8f8f4";
      $("#theme-toggle").setAttribute("aria-label", dark ? "Açık temaya geç" : "Koyu temaya geç");
    };
    $("#theme-toggle").addEventListener("click", () => { document.body.classList.toggle("dark-theme"); localStorage.setItem("etkinlik-takip-theme", document.body.classList.contains("dark-theme") ? "dark" : "light"); update(); });
    update();
  };
  ...
  const init = () => {
    ...
    auth.onAuthStateChanged(async (user) => {
      ...
      database.ref("users").on("value", (snapshot) => {
        userDirectory.clear();
        snapshot.forEach((child) => { const value = child.val() || {}; if (value.email && value.role !== "admin") userDirectory.set(child.key, { key: child.key, email: String(value.email), approved: value.approved === true, blocked: value.blocked === true, createdAt: Number(value.createdAt || 0) }); });
        renderUsers();
